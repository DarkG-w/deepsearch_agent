...
async def run_deep_agent(task_query,session_id,history_context: str = ""):
    ...
    """
    monitor._safe_print(f"当前会话的main_agent开始执行了！ 会话id:{session_id}")
                ...
                if messages and isinstance(messages,list):
                    ...
                    if node_name == 'model':
                        if last_msg.tool_calls:
                            ...
                            # 最终结果
                            monitor._safe_print(f"主智能体执行结果，最终结果：{last_msg.content[:100]}")
                            monitor.report_task_result(last_msg.content)
                            final_result = str(last_msg.content)

    except Exception as e :
        try:
            error_file = session_dir / "agent_error_trace.log"
